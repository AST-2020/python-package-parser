...
import json
import os
from library.struktur_implementation import Structure
...
def read_directory(directory, local_path, struct):
    for entry in os.scandir(directory):
        ...
        struct.module_path = struct.module_path.replace(local_path.replace("/", "."), "")
...
def parse_package(package_name):
    ...
    local_path_to_delete = library_local_path.rsplit(package_name, 1)[0]
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        if parsed_data.cls_name is None:
            for default in node.args.defaults:
                ...
                # print(args[count])
                # print(default.__dir__()[0])
                # print(getattr(default, default_attribute))
...
if __name__ == '__main__':
    ...
    test_json_object = parsed_data.toJSON(parsed_data.dict)
    
    # to write our json data to a txt file
    # parsed_data.convert_to_json("testTextFile")
