...
def _add_tf_shape(attr_dict, ints):
    ...
    attr_dict['_output_shapes'].package.shape.extend([shape_proto])
...
def _set_tf_attr(attr_dict, arg):
    ...
    if arg.HasField("s"):
        ...
        return
    if arg.floats:
        attr_dict[k].package.f.extend(arg.floats)
        return
    if arg.ints:
        attr_dict[k].package.i.extend(arg.ints)
        return
    if arg.strings:
        attr_dict[k].package.s.extend(
