...
def check_type(call: FunctionCall, message_manager: MessageManager):
    ...
    kw_args = [(kw_arg.name, kw_arg.type) for kw_arg in call.keyword_arg]  # keyword argumets from user
    positional_args = [pos_arg.type for pos_arg in call.positional_arg]  # arguments from user without name
    ...
    for i in range(len(kw_args)):
        if kw_args[i][0] in structure_args.keys():
            if structure_args.get(kw_args[i][0]) is not None and structure_args.get(kw_args[i][0]) is not Any and structure_args.get(kw_args[i][0]) == kw_args[i][1]:
                ...
                # Error message
                new_message = Message(call.location, "Error in type_hint: Must be in " + structure_args.get(kw_args[i][0]))
                message_manager.add_message(new_message)
        else:
            # it means that there is an error in arg_name
            print("error in arg_name or type_hint")
            new_message = Message(call.location, "Error in arg_name or type_hint")
            message_manager.add_message(new_message)
            message_manager.print_messages()
            return
            # return is encouraged (problem in kw_args can cause further problems) MUST BE TESTED!!
    ...
    for i in range(len(positional_args)):
        if structure_args[i] is not None and structure_args[i] is not Any and structure_args[i] != positional_args[i]:
            print("error in arg_name or type_hint")
            new_message = Message(call.location, "Error in type_hint: Must be in " + structure_args[i])
            message_manager.add_message(new_message)
